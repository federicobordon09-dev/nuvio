    ...
    >
      ...
      {(studyType || documentType) && (
        <div className="mb-4 flex flex-wrap items-center gap-2">
          {studyType && (
            <span className="rounded-full bg-primary-muted px-3 py-1 text-[12px] font-medium text-primary">
              {getStudyTypeLabel(studyType)}
            </span>
          )}
          {documentType && (
            <span className="rounded-full bg-muted/50 px-3 py-1 text-[12px] font-medium text-muted-foreground">
              {documentType}
            </span>
          )}
        </div>
      ...
      <p className="text-[15px] leading-[1.65] text-foreground">{summary}</p>
    </section>
  );
}
